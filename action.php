...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM tv_series";
	$run_query = mysqli_query($con,$category_query) or die(mysqli_error($con));
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>TV Series</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$cid = $row["series_id"];
			$cat_name = $row["series_title"];
			echo "
...
if(isset($_POST["brand"])){
	$brand_query = "SELECT * FROM movies";
	$run_query = mysqli_query($con,$brand_query);
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>movies</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["movie_id"];
			$brand_name = $row["movie_title"];
			echo "
...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			$pro_cat   = $row['series_id'];
			$pro_brand = $row['movie_id'];
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["cat_id"];
		$sql = "SELECT * FROM products WHERE series_id = '$id'";
	}else if(isset($_POST["selectBrand"])){
		$id = $_POST["brand_id"];
		$sql = "SELECT * FROM products WHERE movie_id = '$id'";
	}else {
...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.product_id AND b.ip_add='$ip_add' AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="shoppingcart@movietore.com">
